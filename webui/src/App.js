    ...
    useEffect(() => {
        ...
        function getTouchPos(canvas, e) {
            ...
            return {
                x:
                    ...
                    dpi,
                y:
                    (e.targetTouches[0].pageY - rect.top - window.pageYOffset) *
                    dpi,
            };
        }
        ...
        const render = (timestamp) => {
            ...
            if (curValues.length < MAX_SIZE) {
                ...
                currentValue =
                    curValues[
                        (((oldest - 1) % MAX_SIZE) + MAX_SIZE) % MAX_SIZE
                    ][index];
            }
        ...
        const render = (timestamp) => {
            ...
            for (let i = 1; i * minor_division < 1023; ++i) {
                ...
                drawDashedLine(
                    ...
                    spacing,
                    box_height -
                        (box_height * (i * minor_division)) / 1023 +
                        spacing,
                    box_width + spacing
                );
            }
            ...
            for (let i = 0; i < numSensors; ++i) {
                if (display[i]) {
                    ...
                    ctx.moveTo(
                        spacing,
                        box_height -
                            (box_height * curThresholds[i]) / 1023 +
                            spacing
                    );
                    ctx.lineTo(
                        box_width + spacing,
    ...
    function AddProfile(e) {
        ...
        if (e.keyCode === 13) {
            emit([
                "add_profile",
                e.target.value,
                webUIDataRef.current.curThresholds,
            ]);
            // Reset the text box.
            e.target.value = "";
        }
